package MsneddonContigFilter4::MsneddonContigFilter4Client;
...
MsneddonContigFilter4::MsneddonContigFilter4Client
...
sub new
{
    ...
    my $self = {
	client => MsneddonContigFilter4::MsneddonContigFilter4Client::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a MsneddonContigFilter4.FilterContigsParams
$output is a MsneddonContigFilter4.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is a MsneddonContigFilter4.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is a MsneddonContigFilter4.assembly_ref
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "MsneddonContigFilter4.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "MsneddonContigFilter4.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "MsneddonContigFilter4.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for MsneddonContigFilter4::MsneddonContigFilter4Client\n";
    }
    if ($sMajor == 0) {
        warn "MsneddonContigFilter4::MsneddonContigFilter4Client version is $svr_version. API subject to change.\n";
    }
}
...
package MsneddonContigFilter4::MsneddonContigFilter4Client::RpcClient;
